...
def generate_markdown_table(game_scores: list[GameScore]) -> str:
    ...
    for game_score in game_scores:
        ...
        if game_score.away_pitcher_nerd_score is not None and game_score.away_starter:
            # Pitcher heading: "### Pitcher Name, Team Name"
            away_pitcher_heading = f"{game_score.away_starter}, {game_score.away_team}"
        ...
        if game_score.home_pitcher_nerd_score is not None and game_score.home_starter:
            # Pitcher heading: "### Pitcher Name, Team Name"
            home_pitcher_heading = f"{game_score.home_starter}, {game_score.home_team}"
...
def generate_game_detail_section(
    ...
    # Add description section if requested and available
    if include_descriptions and game_score.game_description is not None:
        lines.extend(
            [
                "### Summary",
                "",
                game_score.game_description,
                "",
            ]
        )
        ...
        if game_score.game_description_sources:
            ...
            source_links = []
            for i, source in enumerate(game_score.game_description_sources, 1):
                if "url" in source:
                    source_links.append(f"[{i}]({source['url']})")
                else:
    ...
    if game_score.away_starter and game_score.away_starter != "TBD":
        if game_score.away_pitcher_nerd_stats:
            ...
            lines.append(
                generate_pitcher_no_data_section(
                    game_score.away_starter, game_score.away_team
                )
            )
    ...
    if game_score.home_starter and game_score.home_starter != "TBD":
        if game_score.home_pitcher_nerd_stats:
            ...
            lines.append(
                generate_pitcher_no_data_section(
                    game_score.home_starter, game_score.home_team
                )
            )
