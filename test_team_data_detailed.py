...
def test_detailed_team_stats():
    ...
    try:
        ...
        try:
            ...
            potential_functions = [
                'team_results', 'standings', 'schedule_and_record',
                'team_stats', 'league_batting_stats', 'league_pitching_stats'
            ]
        ...
        for stat, info in tnerd_requirements.items():
            ...
            if team_batting is not None:
                for source in info['potential_sources']:
                    matching_cols = [col for col in team_batting.columns 
                                   if source.lower() in col.lower()]
                    if matching_cols:
                        found_columns.extend([(col, 'batting') for col in matching_cols])
            
            if team_pitching is not None:
                for source in info['potential_sources']:
                    matching_cols = [col for col in team_pitching.columns 
                                   if source.lower() in col.lower()]
                    if matching_cols:
                        found_columns.extend([(col, 'pitching') for col in matching_cols])
            
            if team_fielding is not None:
                for source in info['potential_sources']:
                    matching_cols = [col for col in team_fielding.columns 
                                   if source.lower() in col.lower()]
                    if matching_cols:
                        found_columns.extend([(col, 'fielding') for col in matching_cols])
            ...
            if found_columns:
                ...
                print(f"  ✗ No obvious matches found in available data")
        ...
        if team_batting is not None:
            ...
            key_batting_cols = ['Team', 'G', 'PA', 'HR', 'R', 'RBI', 'wRC+', 'wOBA', 'Off']
            available_key_cols = [col for col in key_batting_cols if col in team_batting.columns]
            ...
            batting_runs_cols = [col for col in team_batting.columns 
            ...
            # Home run rate
            hr_cols = [col for col in team_batting.columns 
            ...
            # Baserunning
            baserunning_cols = [col for col in team_batting.columns 
        ...
        if team_pitching is not None:
            ...
            bullpen_cols = [col for col in team_pitching.columns 
        ...
        if team_fielding is not None:
            ...
            def_cols = [col for col in team_fielding.columns 
                       if any(term in col.lower() for term in ['def', 'uzr', 'drs', 'fielding'])]
            if def_cols:
        ...
        if team_batting is not None:
            ...
            if 'wRC+' in team_batting.columns and 'PA' in team_batting.columns:
                ...
                sample['Batting_Runs_Approx'] = (sample['wRC+'] - 100) * sample['PA'] / 1000
        ...
        if team_batting is not None and 'Age' in team_batting.columns:
            ...
            print("Age not found in team batting data - may need individual player aggregation")
            print()
        ...
        if team_batting is not None:
            ...
            if 'Off' in sample_teams.columns:
                ...
                tnerd_sample['Batting_Runs'] = (sample_teams['wRC+'] - 100) * sample_teams['PA'] / 1000
        ...
        print("• Team batting, pitching, and fielding data are available from pybaseball")
        print("• Some tNERD components can be calculated directly (HR%, partial batting runs)")
        ...
        print("2. Create functions to aggregate individual player data for missing components")
