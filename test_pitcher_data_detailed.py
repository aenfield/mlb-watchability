    ...
    try:
        ...
        pnerd_mapping = {
            ...
            "vFA": {
                ...
                "alternatives": ["vFA (pi)", "vFA (sc)", "FBv"] if any(col in starters.columns for col in ["vFA (pi)", "vFA (sc)", "FBv"]) else None,
            },
            "Age": {
                "description": "Age",
        ...
        for stat, info in pnerd_mapping.items():
            ...
            if info["found_column"]:
                ...
                available_alts = [alt for alt in info["alternatives"] if alt in starters.columns]
                if available_alts:
                    print(f"  ~ Alternative found: {available_alts[0]}")
                    print(f"    All alternatives: {available_alts}")
                else:
                    print(f"  ✗ Not found (no alternatives available)")
            else:
                print(f"  ✗ Not found")
        ...
        if len(has_all_velocity) > 0:
            print(f"Pitchers with data in all three velocity columns: {len(has_all_velocity)}")
            
            # Show sample comparison
            sample_comparison = has_all_velocity[["Name", "Team"] + velocity_comparison_cols].head(10)
            ...
            print(f"vFA (pi) - vFA (sc): mean={diff_pi_sc.mean():.2f}, std={diff_pi_sc.std():.2f}")
            print(f"vFA (pi) - FBv: mean={diff_pi_fbv.mean():.2f}, std={diff_pi_fbv.std():.2f}")
            print(f"vFA (sc) - FBv: mean={diff_sc_fbv.mean():.2f}, std={diff_sc_fbv.std():.2f}")
        ...
        for field_name, column_name in pnerd_fields.items():
            if column_name in starters.columns:
                ...
                print(f"  Has data: {non_null_count}/{total_pitchers} ({percentage:.1f}%)")
                if null_count > 0:
        ...
        print(f"Pitchers with complete pNERD data: {complete_count} ({complete_count/total_pitchers*100:.1f}%)")
        print(f"Pitchers with incomplete data: {incomplete_count} ({incomplete_count/total_pitchers*100:.1f}%)")
        ...
        if incomplete_count > 0:
            ...
            for field_name, column_name in pnerd_fields.items():
                if column_name != 'KN%' and column_name in starters.columns:
                    ...
                    if missing_count > 0:
                        missing_pitchers = starters[starters[column_name].isna()]['Name'].tolist()
                        print(f"  {field_name}: {missing_count} pitchers missing")
                        if missing_count <= 10:  # Show names if not too many
        ...
        print(f"  KN% has actual data: {kn_non_null}/{total_pitchers} ({kn_non_null/total_pitchers*100:.1f}%)")
        print(f"  KN% will be set to 0: {kn_null}/{total_pitchers} ({kn_null/total_pitchers*100:.1f}%)")
        ...
        for pitcher_name in target_pitchers:
            ...
            if len(pitcher_data) > 0:
                ...
                if "Strikes" in pitcher_row and "Pitches" in pitcher_row:
                    ...
                    print(f"  Strike Rate: {strike_rate:.3f} (calculated from {pitcher_row['Strikes']} strikes / {pitcher_row['Pitches']} pitches)")
                else:
                    print(f"  Strike Rate: Cannot calculate (missing Strikes/Pitches)")
                ...
                if available_velocity:
                    print(f"  Velocity options:")
                    for vel_info in available_velocity:
                        print(f"    {vel_info}")
                else:
                    print(f"  vFA (velocity): N/A mph (no velocity data available)")
                ...
                if "ERA-" in pitcher_row and "xFIP-" in pitcher_row:
                    ...
                    print(f"  Luck: {luck:.1f} (calculated from {pitcher_row['ERA-']} ERA- - {pitcher_row['xFIP-']} xFIP-)")
                else:
                    print(f"  Luck: Cannot calculate (missing ERA- or xFIP-)")
                ...
                similar = starters[starters["Name"].str.contains(pitcher_name.split()[-1], case=False, na=False)]
                if len(similar) > 0:
...
if __name__ == "__main__":
    success = test_detailed_pitcher_stats()
