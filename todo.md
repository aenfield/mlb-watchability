...
43. ~~The folks at awfulannouncing.com have released their 2025 broadcaster ratings, so I'll give adding a new component to tNERD a shot (with credit to the site). The data itself is in data/broadcasters.tv.2025.csv - I want to use the Team field (should be the same as the FanGraph's data and so no mapping needed ) - and the 'Rating' field (the others aren't needed). The contribution to the overall tNERD is the z-score of the 'Rating' field, capped on the low side at zero, so any scores with a z-score of below zero are just treated as contributing zero to the tNERD (in a similar way to how age and payroll are treated, with the difference that higher scores are better for broadcasters). Higher is better. Also update the team detail tables by adding a new column called 'TV' after 'Luck' and before 'Constant', and include the broadcaster component there.~~

44. ~~Following on from #43, I want to recommend a broadcast for each game. The recommended broadcast is the team with the highest broadcaster rating. Add a new line to the game markdown output after the top-level 'Team 1 @ Team 2, 11:20a' line and before the 'Summary' line that uses this format, where the recommended broadcast is the team with the highest broadcaster rating: "Recommended broadcast: San Diego Padres (3.47 rating, from [Awful Announcing](awfulannouncing.com))". We want to use the underlying raw source rating number, not the broadcaster tNERD component (the one that's the z-score capped on the low side at zero), so depending on how #43 is implemented, this might also require piping through the raw broadcaster rating to the point where it's in the game score or where it's accessible to the code generating the markdown file.~~

45. I've now added radio broadcaster ratings too, in data/broadcasters.radio.2025.csv, in addition to the TV broadcaster data I'm already using and that was added above in #43 and #44. In this TODO item, I want to update tNERD to include a 'Radio' component that works identically to the 'TV' component, and update the displayed team tables to include a 'Radio' column (after the 'TV' column) - this matches basically #43 above. I also want to update the game markdown output to recommend a radio broadcast. First, the existing text should be updated to say 'Recommended TV broadcast'. Then, I want a new line of text w/ the same bolding and starting with 'Recommended radio broadcast', and the full team name of the team with the higher radio component. The link behind the score should go to https://awfulannouncing.com/orig/2025-mlb-local-radio-booth-rankings-miller-rose-hughes-hamilton.html. In all of these changes you should be able to do things very similarly to how the TV component is already calculated and displayed and used - however, be sure not to repeat yourself and instead keep DRY... if you need to use the same code in multiple places, extract it to a method first so you aren't copying and pasting.

46. Consider limiting allowed sites for sources via the Anthropic API to a few to prefer content from places like FanGraphs and hopefully other analytical sites with recent info (would also want to make sure there's enough general and recent game-specific info).
