...
45. Consider limiting allowed sites for sources via the Anthropic API to a few to prefer content from places like FanGraphs and hopefully other analytical sites with recent info (would also want to make sure there's enough general and recent game-specific info).
