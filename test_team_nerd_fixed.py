...
def calculate_tnerd_components(team_data: pd.DataFrame) -> pd.DataFrame:
    ...
    tnerd_data['payroll'] = base_payroll + (np.random.random(len(tnerd_data)) - 0.5) * 2 * payroll_variation
...
def display_sample_results(tnerd_data: pd.DataFrame) -> None:
    ...
    for i, (idx, team) in enumerate(sorted_data.head(3).iterrows()):
        ...
        print(f"   Bullpen xFIP: {team['bullpen_xfip']:.2f} (z_flipped: {team['z_bullpen_xfip_flipped']:.2f})")
        print(f"   Defense: {team['defensive_runs']:.1f} (z: {team['z_defensive_runs']:.2f})")
        print(f"   Payroll: ${team['payroll']:.0f}M (adj: {team['adjusted_payroll']:.2f})")
...
def test_specific_teams(tnerd_data: pd.DataFrame) -> None:
    ...
    for team_code in target_teams:
        ...
        if len(team_data) > 0:
            ...
            print(f"  Age={team['age']:.1f}, Payroll=${team['payroll']:.0f}M, Luck={team['luck']:.1f}")
            print(f"  Record: {team['W']:.0f}-{team['L']:.0f}")
        else:
...
def demonstrate_with_sample_teams(tnerd_data: pd.DataFrame) -> None:
    ...
    print(f"  Strengths: Batting={team1['batting_runs']:.1f}, Defense={team1['defensive_runs']:.1f}")
    print(f"")
    print(f"{team2['Team']} tNERD: {team2['tnerd_score']:.2f}")
    print(f"  Record: {team2['W']:.0f}-{team2['L']:.0f}")
    print(f"  Strengths: Batting={team2['batting_runs']:.1f}, Defense={team2['defensive_runs']:.1f}")
    print(f"")
    ...
    print(f"(Note: This would be combined with pNERD scores to calculate final gNERD)")
...
def main():
    ...
    try:
        ...
        print("• Batting runs, HR%, baserunning, defense, and age are directly available")
