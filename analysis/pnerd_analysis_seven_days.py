...
def collect_games_and_pitchers(dates: list[str]) -> list[dict[str, Any]]:
    """
...
def add_pnerd_scores(game_pitcher_data: list[dict[str, Any]]) -> pd.DataFrame:
    """
...
def analyze_pnerd_distribution(df: pd.DataFrame) -> None:
    ...
    if len(high_outliers) > 0:
        ...
        for _, row in top_performers.sort_values(
            "pnerd_score", ascending=False
        ).iterrows():
            print(
                f"  {row['pitcher_name']} ({row['pitcher_team']}): {row['pnerd_score']:.3f}"
            )
