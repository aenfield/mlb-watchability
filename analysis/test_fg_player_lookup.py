...
def display_results(results_df: pd.DataFrame, title: str):
    ...
    for idx, row in results_df.iterrows():
        ...
        print(
            f"   MLB Debut: {row.get('mlb_played_first', 'N/A')} - {row.get('mlb_played_last', 'N/A')}"
        )
    ...
    for _, player_row in lookup_results.iterrows():
        ...
        try:
            ...
            if not pitcher_match.empty:
                ...
                print(
                    f"   🥎 Pitching: GS={p_row.get('GS', 0)}, IP={p_row.get('IP', 0):.1f}, ERA={p_row.get('ERA', 0):.2f}"
                )
            else:
        ...
        try:
            ...
            if not batter_match.empty:
                ...
                print(
                    f"   🏏 Batting: G={b_row.get('G', 0)}, AB={b_row.get('AB', 0)}, AVG={b_row.get('AVG', 0):.3f}"
                )
            else:
