...
def collect_team_data() -> pd.DataFrame:
    ...
    for team_abbr, team_stats in team_stats_dict.items():
        ...
        if nerd_stats:
            ...
            for field in ["tnerd_score", "z_batting_runs", "z_barrel_rate", 
                         "z_baserunning_runs", "z_fielding_runs", "z_payroll", 
                         "z_age", "z_luck", "adjusted_payroll", "adjusted_age", 
                         "adjusted_luck", "batting_component", "barrel_component", 
                         "baserunning_component", "fielding_component", 
                         "payroll_component", "age_component", "luck_component", 
                         "constant_component"]:
                row[field] = None
...
def analyze_component_contributions(df: pd.DataFrame) -> None:
    ...
    components = [
        "batting_component", "barrel_component", "baserunning_component",
        "fielding_component", "payroll_component", "age_component", 
        "luck_component", "constant_component"
    ]
...
def analyze_team_characteristics(df: pd.DataFrame) -> None:
    ...
    raw_stats = ["batting_runs", "barrel_rate", "baserunning_runs", 
                 "fielding_runs", "payroll", "age", "luck"]
    ...
    for stat in raw_stats:
        if stat in valid_scores.columns:
            ...
            if len(values) > 0:
                ...
                min_val = values.min()
                max_val = values.max()
                print(f"  {stat:18}: {mean_val:7.2f} ± {std_val:6.2f} (range: {min_val:6.2f} to {max_val:6.2f})")
