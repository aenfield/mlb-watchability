...
def main(max_web_searches: int = 1, prompt_filename: str | None = None) -> None:
    ...
    try:
        ...
        if hasattr(message.usage, "input_tokens") and hasattr(
            ...
            ) * 3.00  # $3 per million input tokens
            output_cost = (
                output_tokens / 1_000_000
            ) * 15.00  # $15 per million output tokens
        ...
        # Extract and display web search citations/sources
        web_sources = []
        for block in message.content:
            if hasattr(block, "type") and block.type == "web_search_tool_result":
                if hasattr(block, "content"):
