...
def get_current_stats(player_name: str):
    ...
    try:
        ...
        if not pitcher_match.empty:
            ...
            print(
                f"   🥎 Pitching: GS={p_row.get('GS', 0)}, IP={p_row.get('IP', 0):.1f}, ERA={p_row.get('ERA', 0):.2f}, WHIP={p_row.get('WHIP', 0):.2f}"
            )
        else:
    ...
    try:
        ...
        if not batter_match.empty:
            ...
            print(
                f"   🏏 Batting: G={b_row.get('G', 0)}, AB={b_row.get('AB', 0)}, AVG={b_row.get('AVG', 0):.3f}, OPS={b_row.get('OPS', 0):.3f}"
            )
        else:
