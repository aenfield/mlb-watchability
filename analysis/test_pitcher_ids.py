...
def get_pitcher_mlbam_ids_and_stats(date: str = None):
    ...
    try:
        ...
        pitcher_stats_df = pb.pitching_stats(
            2025, qual=1
        )  # Get all pitchers with at least 1 inning
        ...
        has_mlbam_ids = (
            "mlbam_id" in pitcher_stats_df.columns
            or "key_mlbam" in pitcher_stats_df.columns
        )
        print(f"PyBaseball data contains MLBAM IDs: {has_mlbam_ids}")
        if has_mlbam_ids:
            mlbam_col = (
                "mlbam_id" if "mlbam_id" in pitcher_stats_df.columns else "key_mlbam"
            )
        ...
        for i, game in enumerate(raw_schedule, 1):
            print(
                f"Game {i}: {game.get('away_name', 'Unknown')} @ {game.get('home_name', 'Unknown')}"
            )
            ...
            print(
                f"  Away pitcher: {away_pitcher_name} (ID from schedule: {away_pitcher_id})"
            )
            print(
                f"  Home pitcher: {home_pitcher_name} (ID from schedule: {home_pitcher_id})"
            )
                ...
                if not away_pitcher_id:
                    try:
                        ...
                        if lookup_results:
                            ...
                            pitcher_data["lookup_full_name"] = best_match.get(
                                "fullName"
                            )
                            print(
                                f"    Found: {best_match.get('fullName')} (ID: {best_match.get('id')})"
                            )
                        else:
        ...
        print(
            "Testing both name matching and FanGraphs ID matching for today's pitchers..."
        )
        print("=" * 80)
        ...
        for pitcher in all_pitcher_data:
            ...
            if not name_matches.empty:
                ...
                print(
                    f"    ✓ Found by exact name: GS={name_match_result['gs']}, IP={name_match_result['ip']:.1f}"
                )
            else:
            ...
            try:
                ...
                id_lookup = pb.playerid_reverse_lookup(
                    [pitcher["mlbam_id"]], key_type="mlbam"
                )
                ...
                if not id_lookup.empty and "key_fangraphs" in id_lookup.columns:
                    ...
                    if pd.notna(fangraphs_id):
                        ...
                        if "playerid" in pitcher_stats_df.columns:
                            fg_matches = pitcher_stats_df[
                                pitcher_stats_df["playerid"] == fangraphs_id
                            ]
                        elif "IDfg" in pitcher_stats_df.columns:
                            fg_matches = pitcher_stats_df[
                                pitcher_stats_df["IDfg"] == fangraphs_id
                            ]
                        else:
                        ...
                        if not fg_matches.empty:
                            ...
                            print(
                                f"    ✓ Found by FanGraphs ID: GS={fangraphs_match_result['gs']}, IP={fangraphs_match_result['ip']:.1f}"
                            )
                        else:
            ...
            if name_match_result and fangraphs_match_result:
                if (
                    name_match_result["gs"] == fangraphs_match_result["gs"]
                    and abs(name_match_result["ip"] - fangraphs_match_result["ip"])
                    < 0.1
                ):
                    print("    ✅ Both methods agree!")
                else:
                    print(
                        f"    ⚠ Methods disagree! Name: GS={name_match_result['gs']}, IP={name_match_result['ip']:.1f} vs FG: GS={fangraphs_match_result['gs']}, IP={fangraphs_match_result['ip']:.1f}"
                    )
        ...
        ids_from_schedule = len(
            [p for p in all_pitcher_data if p["source"] == "schedule"]
        )
        ...
        stats_matches = len(
            [p for p in all_pitcher_data if p["pybaseball_match"] is not None]
        )
        stats_missing = len(
            [p for p in all_pitcher_data if p["pybaseball_match"] is None]
        )
        ...
        for pitcher in all_pitcher_data:
            id_status = (
                f"MLBAM ID: {pitcher['mlbam_id']}" if pitcher["mlbam_id"] else "NO ID"
            )
            print(
                f"{pitcher['name']} ({pitcher['team']}) - {id_status} [{pitcher['source']}]"
            )
            if (
                pitcher.get("lookup_full_name")
                and pitcher["lookup_full_name"] != pitcher["name"]
            ):
                print(f"  Full name from lookup: {pitcher['lookup_full_name']}")
            if pitcher["pybaseball_match"]:
                print(
                    f"  PyBaseball: GS={pitcher['gs']}, IP={pitcher['ip']:.1f} [{pitcher['pybaseball_match']}]"
                )
            else:
        ...
        for pitcher in all_pitcher_data:
            if pitcher["gs"] is not None and pitcher["ip"] is not None:
                print(
                    f"{pitcher['name']:<25} GS: {pitcher['gs']:>2}, IP: {pitcher['ip']:>5.1f}"
                )
            else:
